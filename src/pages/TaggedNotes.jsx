...
function TaggedNotes() {
  ...
  const taggedNotes = notes.filter(
    (note) => !note.archived && note.tags?.includes(activeTag)
  );
  const sortedTaggedNotes = [...taggedNotes].sort((a, b) => b.id - a.id);

  const [selectedNoteId, setSelectedNoteId] = useState(
    sortedTaggedNotes.length > 0 ? sortedTaggedNotes[0].id : null
  );
  ...
  const handleCancelCreate = () => {
    setIsCreating(false);
    setNewNote({ title: "", content: "", tags: [activeTag], archived: false });
    if (sortedTaggedNotes.length > 0) {
      setSelectedNoteId(sortedTaggedNotes[0].id);
    } else {
      ...
      <div className={styles.rightSide}>
        ...
        <div className={styles.contentContainer}>
          <div className={styles.notesList}>
            ...
            {sortedTaggedNotes.length > 0
              ? sortedTaggedNotes.map((note) => (
                  <NoteCard
                    key={note.id}
                    note={note}
              ...
              : !isCreating && (
                  <div className={notesStyles.emptyState}>
                    <p>
                      No notes with the "{activeTag}" tag yet. Create a new note
                      and tag it to see it here.
                    </p>
                  </div>
                )}
          </div>
